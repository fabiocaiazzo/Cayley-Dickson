...
import { POSITIVE_TRIPLETS, palette } from '../math/data.js';
...
function createTripletPath(triplet, colorHex) {
    ...
    // Uso il nuovo modulo centralizzato
    const tubeData = createTubeGeometry(p1, p2, p3);
    const material = createTubeMaterial(colorHex, tubeData.isCollinear);
    
    const mesh = new THREE.Mesh(tubeData.geometry, material);
    scene.add(mesh);

    const hitGeom = new THREE.TubeGeometry(tubeData.curve, 32, 0.25, 3, !tubeData.isOpenCurve);
    ...
    tripletVisuals.push({ 
        ...
        hitMesh: hitMesh, 
        color: colorHex, 
        isCollinear: tubeData.isCollinear 
    });
}
...
export function updateSceneVisibility(limitIndex) {
    // Bug 19: Se siamo in modalità rotazione, blocca il ripristino dei nodi del grafo
    const rotUI = document.getElementById('rotation-ui');
    if (rotUI && rotUI.style.display === 'flex') {
        return; 
    }
    ...
    const tBtns = document.querySelectorAll('.triplet-btn');
    tripletVisuals.forEach((t, i) => {
        ...
        if (tBtns[i]) tBtns[i].style.display = isVisible ? 'block' : 'none';
    });
}
