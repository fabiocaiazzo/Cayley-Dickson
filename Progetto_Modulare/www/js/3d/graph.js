...
import { currentAlgState, tripletButtons, fanoButtons } from '../core/state.js';
...
export function animateCameraToFanoPlane(planeIdx) {
    ...
    const DIST = (isMobileNow && zoomedOutPlanes.includes(planeIdx)) ? 38 : 24;
...
export function visualizeFanoPlane(planeIdx, fromCalculator = false) {
    if (isRotationMode) toggleRotationMode();
    if (!fromCalculator && resetZeroDivisorUI) resetZeroDivisorUI();
