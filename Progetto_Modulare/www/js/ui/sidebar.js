import { container, camera, renderer, labelRenderer, tripletVisuals } from '../3d/scene.js';
import { visualizeTripletByIndex, activeFanoIndex, animateCameraToDefault, visualizeFanoPlane } from '../3d/graph.js';
import { tripletButtons, fanoButtons } from '../core/state.js';
...
import { POSITIVE_TRIPLETS, FANO_PLANES } from '../math/data.js';
...
export function initSidebar() {
    ...
    function toggleSidebar() {
        ...
        if (sidebar.classList.contains('open') && !isTop) {
            sidebar.style.zIndex = zManager.next();
            return;
        }
        ...
        if (sidebar.classList.contains('open')) sidebar.style.zIndex = zManager.next();
        setTimeout(() => {
    ...
    function stopResize() {
        if (!isResizing) return;
        isResizing = false;
        unbindResizer(); // K2: Rimuove i listener al rilascio
    ...
    // Ottimizzazione K2: i listener globali vengono attaccati dinamicamente
    const onMouseMove = (e) => doResize(e.clientX);
    const onTouchMove = (e) => { if (isResizing) e.preventDefault(); doResize(e.touches[0].clientX); };
    ...
    // Mouse & Touch trigger events
    resizer.addEventListener('mousedown', (e) => { bindResizer(); startResize(e); });
    resizer.addEventListener('touchstart', (e) => { bindResizer(); startResize(e); e.preventDefault(); }, { passive: false });
    ...
    setTimeout(() => { if (isMobile()) updateSidebarCarousel(0, false); }, 100);

    window.addEventListener('resize', () => {
        updateSidebarCarousel(0, false);
    });
    ...
    sidebarTabsWrapper.addEventListener('touchstart', (e) => {
        ...
        // Mantieni il fix della Tabella e dell'intera Tab Rotazioni (spiegazione + lista): 
        // se tocchi questi elementi, lo swipe non si avvia, permettendoti di scrollarli liberamente.
        if (e.target.closest('#sedenion-table') || e.target.closest('#tab-rotations')) return;
        ...
        updateSidebarCarousel(0, false); // Ferma eventuali animazioni in corso
    }, { passive: true });
    ...
    sidebarTabsWrapper.addEventListener('touchmove', (e) => {
        ...
        if (sbSwipeDirectionDetermined) {
            ...
            if (sbIsHorizontalSwipe) {
                ...
                updateSidebarCarousel(sbSwipeCurrentX - sbSwipeStartX, false);
            } else {
    ...
    const endSidebarDrag = () => {
        ...
        if (sbSwipeDirectionDetermined && !sbIsHorizontalSwipe) {
            updateSidebarCarousel(0, true); // Assicura allineamento
            return;
        }
...
export function getVisibleTabs() {
    return Array.from(document.querySelectorAll('.sidebar-tabs .tab-btn')).filter(btn => {
        return (window.getComputedStyle(btn).display !== 'none' || btn.classList.contains('active')) && btn.id !== 'close-sidebar-btn';
    });
}

export function updateSidebarCarousel(dragDelta = 0, animate = false) {
    if (!isMobile()) {
