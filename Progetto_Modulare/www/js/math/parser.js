...
import { vecAdd, vecSub, vecMul, vecInv, vecNorm, vecConj, vecScale, vecPow, vecDot, vecComm, vecAssoc, createBaseVec, createRandomVec, computeKernelBasis } from './algebra.js';
import { t } from '../core/i18n.js';

export const EPSILON = 1e-9;

export const createZeroVector = () => new Array(32).fill(0);

export const storedVars = {
    a: new Array(32).fill(0), b: new Array(32).fill(0), c: new Array(32).fill(0),
    d: new Array(32).fill(0)
};
...
export function validateAssociativity(exprStr) {
    ...
    const getVectorValue = (token) => {
        ...
        if (token.startsWith('e_')) {
            ...
            if (!isNaN(idx) && idx >= 0 && idx < 32) v[idx] = 1;
            return v;
        }
        if (!isNaN(parseFloat(token))) {
            const v = new Array(32).fill(0); v[0] = parseFloat(token);
            return v;
        }
        return null;
    };
    ...
    const getMaxIndex = (v) => {
        ...
        if (Array.isArray(v)) {
            for (let i = 31; i >= 0; i--) if (Math.abs(v[i]) > EPSILON) return i;
            return 0;
        }
        return 31;
    };
    ...
    const validateChain = (chain) => {
        ...
        const hasZero = chain.some(t => {
            ...
            return v && v.every(x => Math.abs(x) < EPSILON);
        });
        if (hasZero) return;
        ...
        // Evita di lanciare falsi errori di ambiguità se ci sono token (variabili) ancora in digitazione e sconosciuti
        const isUnknown = (token, val) => {
            if (token && typeof token === 'object' && token.isGroup) return false;
            return val === null;
        };
        if (isUnknown(A, vA) || isUnknown(B, vB) || isUnknown(C, vC)) throw new Error("Unknown");
    ...
    let tokens = exprStr.split(/([+\-*/^(),]|\bcomm\b|\bdot\b|\bnorm\b|\bconj\b|\binv\b|\brand\b)/).filter(t => t);
    let level = 0;
    let chain = [];
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (['norm', 'conj', 'inv', 'comm', 'dot', 'rand'].includes(t)) {
        ...
        if (t === '(') {
            ...
            if (level > 0) {
                ...
                if (t === '+' || t === '-' || t === ',') {
                    ...
                    const isZero = v && v.every(x => Math.abs(x) < EPSILON);
...
export function evaluateExpression(exprStr, trace = false) {
    ...
    exprStr = processedStr.replace(/\[/g, 'comm(').replace(/\]/g, ')')
        .replace(/</g, 'dot(').replace(/>/g, ')');
    const rawTokens = exprStr.replace(/\s+/g, '').split(/([+\-*/^(),]|norm|conj|inv|comm|dot|e_|rand)/).filter(t => t);
    let tokens = [];
    ...
    for (let i = 0; i < tokens.length; i++) {
        if (tokens[i] === 'comm' || tokens[i] === 'dot') {
            const isDot = tokens[i] === 'dot';
            let localLevel = 0;
            let commCount = 0;
            ...
            if (isDot) {
                if (commCount === 1) tokens[i] = 'dot2';
                else throw "Il prodotto scalare richiede esattamente due argomenti separati da virgola.";
            } else {
    ...
    const evalStack = [];
    const getV = (tParam) => {
        if (tParam === undefined) throw new Error("Syntax");
        ...
        if (ops[token]) {
            ...
            if (token === '~') {
                ...
                if (token === 'comm2') {
                    ...
                    const a = getV(evalStack.pop());
                    res = ops[token].fn(a, b);
                    args = [a, b]; opLabel = "&lang;a,b&rang;";
                } else {
                    const a = getV(evalStack.pop());
                    res = ops[token].fn(a);
